...
1. X-SAMPA拉丁字母+`\`即为其本字的另一个变体（如`j\`、`s\`、`z\`）。

1. X-SAMPA小写字母+`` ` ``即为其retroflex版本（如``s` ``、``z` ``）。

1. 角标/上下标以`_`+ASCII字符形式呈现，ASCII字符即与相应IPA相似。
